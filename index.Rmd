...
dat <- read_sheet("1h3a1H4UwZHDUUj_s6MWUVDrhn_qqlCm6PTM8lsaHMWA",sheet="RESERVATIONS", col_type="ccccccDcDcnnnnnnnccccccccccc")
#head(dat)
...
for(i in 1:nrow(dat_confirmed))
{
  ...
  if(tmp$`Arrival Date`==tmp$`Departure Date`)
  {
    daily_guests[[i]] <- data.frame(Name=tmp$`First Name`, Last_Name = tmp$`Last Name`,Date=tmp$`Arrival Date`[1],
                                      Adults=tmp$`# people [adults]`[1],Children= tmp$`# [kids u12]`[1], Arrive_today="Yes", Departure="Today", DR_MC=tmp$`Dietry Restrictions`[1])     
  } else {
  daily_guests[[i]] <- data.frame(Name=tmp$`First Name`,Last_Name = tmp$`Last Name`, Date=seq(tmp$`Arrival Date`, tmp$`Departure Date`, by=1 ),
             Adults=tmp$`# people [adults]`[1],Children= tmp$`# [kids u12]`[1],Arrive_today=logic_string[as.numeric(tmp$`Arrival Date`==t)+1], Departure=as.character(tmp$`Departure Date`[1]), Diet=tmp$`Dietry Restrictions`[1])
    }
}
...
tmp <- tmp[, c("First Name", "Last Name", "Booking status", "Arrival Date", "# \nNights", "# people [adults]", "# [kids u12]")]
